...
paths.build = function (settings, routes) {
    ...
    let routeData = {
        ...
        x_summary: Hoek.reach(routeOptions, 'nickname') || null,
        ...
        groups: Hoek.reach(routeOptions, 'group') || route.group || null,
...
paths.properties = function (settings, routes) {
    ...
    let path = internals.removeBasePath( route.path, settings.basePath );
    ...
    var pathParams = Properties.toParameters(internals.getJOIObj(route, 'pathParams'), swagger.definitions, 'path');
    ...
    pathObj[path]['x-summary'] = out['x-summary'];
    delete out['x-summary'];
 ...
 * @return {String}
 */
internals.removeBasePath = function (path, basePath) {

    if (basePath !== '/' && Utilities.startWith(path, basePath)) {
